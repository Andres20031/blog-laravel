    ...
    <div class="max-w-7x1 mx-auto px-2 sm:px-6 lg:px-8 container py-8">
        <div class="grid grid-cols-1 md:grid-cols-2  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4">
            @foreach ($posts as $post)
                <article class="w-full h-80 bg-cover bg-center @if ($loop->first) md:col-span-3 @endif "
                        ...
                        <div>
                            @foreach ($post->tags as $tag)
                                <a href="{{route('posts.show', $post)}}"
                                    class="inline-block px-3 h-6 bg-red-600 text-white rounded-full">{{ $tag->name }}</a>
                            @endforeach
